...
const Navbar = () => {
  ...
  const renderNavLinks = (isSecondary) => (
    <ul className={`list-none ${isSecondary ? "flex sm:hidden flex-col gap-2" : "hidden sm:flex"} ${isSecondary ? "text-center" : "flex-row gap-6"}`}>
      {navLinks.map((link) => (
        <li
          key={link.id}
          className={`${
  ...
  return (
    <>
      <nav className="w-full flex items-center py-3 fixed top-0 z-20" style={{ background: `url(${noiseBackground})`, backgroundColor: 'rgba(0, 0, 0, 0.75)', backgroundBlendMode: 'overlay' }}>
        <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
          ...
          <div className="sm:hidden flex flex-1 justify-end items-center">
            ...
            <div className={`p-4 absolute top-14 right-0 mx-2 my-2 min-w-[120px] z-10 ${toggle ? "flex bg-black bg-opacity-75 rounded-lg" : "hidden"}`} style={{ background: `url(${noiseBackground})`, backgroundColor: 'rgba(0, 0, 0, 0.75)', backgroundBlendMode: 'overlay' }}>
              {renderNavLinks(true)}
            </div>
          </div>
        </div>
