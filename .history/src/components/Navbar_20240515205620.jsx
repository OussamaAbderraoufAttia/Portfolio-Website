...
const Navbar = () => {
  ...
  const renderNavLinks = (isSecondary) => (
    <ul className={`list-none ${isSecondary ? "flex sm:hidden flex-col" : "hidden sm:flex flex-col"} gap-4`}>
      {navLinks.map((link) => (
        <li
          key={link.id}
          className={`${
            active === link.title && !isSecondary ? "text-white" : isSecondary ? "text-secondary" : "text-white"
